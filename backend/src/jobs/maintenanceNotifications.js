...
async function broadcastBrowserPush(doc) {
...
export async function runMaintenanceNotifications() {
  ...
  try {
    ...
    for (const schedule of upcomingSchedules) {
      ...
      const exists = await Notification.findOne({
        ...
        // one notification per schedule per day (D-3 -> D-1)
          reference_date: parseDateOnlyToDate(todayDateOnly),
      });
      ...
      if (!exists) {
        ...
        newCount++;
        io?.emit?.('notification:new', doc.toJSON());
        await broadcastBrowserPush(doc);
      }
    }
    ...
    if (newCount > 0) {
      ...
      io?.emit?.('notification:unread-count', { count: unreadCount });
    }
  } catch (err) {
